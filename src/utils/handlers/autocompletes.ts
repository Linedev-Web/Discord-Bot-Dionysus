...
export default async (client: ExtendedClient) => {
    ...
    const files = readdirSync(autocompletesPath).filter(file => lstatSync(path.join(autocompletesPath, file)).isFile() && (file.endsWith('.js') || (file.endsWith('.ts') && !file.endsWith('.d.ts'))));
