import { PermissionsBitField, ChannelType, TextInputStyle, ComponentType } from "discord.js";
import { Command } from "../../types";

const setWelcomeCommand: Command = {
    run: async ({ client, interaction }) => {
        ...
        if (subCommand === "plugin-state") {
            ...
            if (!channel) return interaction.reply({ content: "Le salon n'existe pas.", ephemeral: true });
            ...
            if (me && 'isTextBased' in channel && channel.isTextBased() && 'permissionsFor' in channel) {
                 if (!channel.permissionsFor(me)?.has([PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.AttachFiles])) {
                    return interaction.reply({
