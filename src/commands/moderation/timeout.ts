...
const convertToMilliseconds = function (value: string): number | null {
    ...
    if (unit === 'min') return amount * 60 * 1000;
    if (unit === 'h' || unit.includes('h')) return amount * 60 * 60 * 1000;
    if (unit === 'd' || unit.includes('d')) return amount * 24 * 60 * 60 * 1000;
    if (unit === 'w' || unit.includes('w')) return amount * 7 * 24 * 60 * 60 * 1000;
...
const timeoutCommand: Command = {
    run: async ({ client, interaction }) => {
        ...
        if (member.isCommunicationDisabled()) return interaction.reply({ content: "Ce membre est déjà mute.", ephemeral: true });
        if (member.id === interaction.user.id) return interaction.reply({ content: 'Vous ne pouvez pas vous rendre muet vous-même.', ephemeral: true });
        if (member.id === client.user?.id) return interaction.reply({ content: 'Vous ne pouvez pas me rendre muet.', ephemeral: true });
        if (member.id === interaction.guild.ownerId) return interaction.reply({ content: "Vous ne pouvez pas rendre muet le propriétaire du serveur.", ephemeral: true });
        ...
        if (!duration) return interaction.reply({ content: "Veuillez indiquer une durée de la mise en sourdine valide. Par exemple '1d' ou '1h'", ephemeral: true });
        if (duration > 28 * 24 * 60 * 60 * 1000) return interaction.reply({ content: "La durée maximale de la mise en sourdine est de 4 semaines.", ephemeral: true });
