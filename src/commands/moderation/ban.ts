...
const banCommand: Command = {
    run: async ({ client, interaction }) => {
        ...
        const member = interaction.options.getMember("utilisateur") as any;
        ...
        if (user.id === client.user?.id) return interaction.reply({ content: "Vous ne pouvez pas me bannir.", ephemeral: true });
        if (user.id === interaction.guild.ownerId) return interaction.reply({ content: "Vous ne pouvez pas bannir le propriétaire du serveur.", ephemeral: true });
        ...
        const messageHistorySeconds: { [key: string]: number } = {
            previous_hour: 60 * 60,
        ...
        interaction.reply({
            content: `J'ai correctement banni **${escapeMarkdown(user.tag)}** du serveur.`, 
            ephemeral: true
        });
