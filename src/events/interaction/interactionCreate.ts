...
const interactionCreateEvent = ({ client, params: [interaction] }: { client: ExtendedClient, params: [Interaction] }) => {
    ...
    if (!interaction.guild || !interaction.channel || !('permissionsFor' in interaction.channel)) return;

    const me = interaction.guild.members.me;
    if (!me || !interaction.channel.permissionsFor(me)?.has([PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.ViewChannel])) return;

    if (interaction.isChatInputCommand()) {
        const command = client.commands.get(interaction.commandName);

        if (!command) return interaction.reply({ content: "Cette commande n'éxiste pas ou n'éxiste plus.", ephemeral: true });
        ...
        if (botPermissions && botPermissions.length > 0 && !interaction.channel.permissionsFor(me)?.has(botPermissions)) {
            return interaction.reply({ 
                content: `Je n'ai pas les permissions suffisantes, veuilliez m'ajouter les permissions suivantes: ${new PermissionsBitField(botPermissions).toArray().map(perm => `\`${perm}\``).join(', ')}.`,
                ephemeral: true
            });
        }
