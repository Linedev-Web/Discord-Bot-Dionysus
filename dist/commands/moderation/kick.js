...
const kickCommand = {
    run: async ({ client, interaction }) => {
        ...
        if (member.id === client.user?.id)
            return interaction.reply({ content: "Vous ne pouvez pas me expulser.", ephemeral: true });
        if (member.id === interaction.guild.ownerId)
            return interaction.reply({ content: "Vous ne pouvez pas expulser le propriétaire du serveur.", ephemeral: true });
        ...
        interaction.reply({
            content: `J'ai correctement expulsé **${(0, discord_js_1.escapeMarkdown)(member.user.tag)}** du serveur`,
            ephemeral: true
        });
    },
    help: {
        description: "Expulse un membre.",
        botPermissions: [discord_js_1.PermissionsBitField.Flags.KickMembers],
        memberPermissions: [discord_js_1.PermissionsBitField.Flags.KickMembers],
